...
import {
  syncFollowupTasks, examChoicesFor, bookingForExam, REPORT_TASK_KIND, examChoiceNote,
...
describe('09 待辦列三行：名字／標籤／一段一行小字', () => {
  ...
  test('系統自己寫的那一句不畫（連 ? 都沒有，ADR-0114），她自己寫的照舊看得到（兩個畫面）', () => {
    for (const [f, t] of [['ui/views/home.js', 't'], ['ui/components/tasklist.js', 'task']]) {
      ...
      assert.ok(!src.includes(`tip(${t}.note`), `${f}：系統那一句 9/24 晚連 ? 都拿掉了`);
...
describe('排查時找到的（第二批）', () => {
  test('客戶詳情「看全部」那張面板的「詳情 ›」接上了', () => {
    ...
    assert.ok(body.includes('btn.dataset.tasklistTask'), '要帶是哪一張（issues/08）');
  });
});
...
describe('11 「這是哪一次健檢」標狀態，只有已完成按得下去', () => {
  ...
  const fuErrors = (draft, stored = null) => validateVisit(draft, {
    customer: { id: 'c1' }, courses: COURSES, entitlements: ENTS,
  ...
  test('被佔走的那一次，丸子上寫「已約」加日期（三個入口同一支）', () => {
    const f2 = { ...fu('e1'), date: '2026-09-10', status: 'confirmed',
      slots: [{ ...fu('e1').slots[0], status: 'confirmed' }] };
    const [first] = examChoicesFor(pair, [...world, f2]);
    assert.equal(examChoiceNote(first), `已約 ${shortDate('2026-09-10')}`);
    assert.equal(examChoiceNote(examChoicesFor(pair, world)[1]), shortStatus('confirmed'));
  });
  ...
  test('三個入口只從按得下去的裡面挑、灰掉的講自己的狀態', () => {
    ...
    for (const f of ['views/schedule.js', 'views/visitEditor.js', 'components/aboveeConfirm.js']) {
      assert.ok(src(f).includes("tip('還沒排過健檢')"), `${f}：一次都沒排過時那一排標題旁邊一顆 ?`);
      assert.ok(!src(f).includes('`已約 ${'), `${f}：「已約」那一句只寫在 examChoiceNote()`);
    }
  });
});

describe('審查修正（第二批）', () => {
  test('舊的寫紀錄（沒有 slotIndexes）：只認要寫紀錄而且做完的那一段，取消與未到的不認', () => {
    const C = {
      rec: { id: 'rec', name: '營養諮詢', category: null, needsRecord: true },
      fu2: { id: 'fu2', name: '二返', category: 'A', needsRecord: true },
    };
    const v = { id: 'v', customerId: 'c1', date: '2026-09-20', status: 'done',
      slots: [
        { courseId: 'rec', startsAt: '09:00', status: 'cancelled' },
        { courseId: 'fu2', startsAt: '10:00', status: 'no_show', attended: false },
        { courseId: 'rec', startsAt: '11:00', status: 'done', attended: true },
      ] };
    const old = { id: 't', visitId: 'v', kind: RECORD_TASK_KIND, autoGenerated: true };
    assert.deepEqual(taskSlots(old, v, C), [2]);
    // 讀取卡片的歸屬是同一支：取消掉的那一段卡片上不列它
    assert.ok(!todosForVisit(v, { tasks: [old], coursesById: C, focusSlot: 0 }).some((r) => r.key === 't'));
  });
});
