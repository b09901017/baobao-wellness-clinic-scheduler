...
import * as treatmentSheets from './views/treatmentSheets.js';
...
// 療程單（issue 14）。三段的那一條不會跟 `/settings/:type` 撞（`router.js` 先比段數）
register('/settings/treatment-sheets', { title: '療程單', nav: false, render: treatmentSheets.render });
register('/settings/treatment-sheets/:customerId', {
  title: '療程單', nav: false,
  render: (el, customerId) => treatmentSheets.renderPerson(el, customerId),
});
