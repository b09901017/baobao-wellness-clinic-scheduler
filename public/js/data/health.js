...
function opFor(fix) {
  ...
  if (fix?.kind === 'setCourseDuration') {
    return {
      op: 'update',
      path: 'config/app/courses',
      id: fix.courseId,
      changes: { durationMin: fix.durationMin },
      note: '資料健檢：課程的時長改成建議值',
    };
  }

  // 點滴品項身上那一格時長（護心抗老 180 分，ADR-0098）。2026-09-16 才加的
  // 欄位，所以既有主檔那一格是空的。
  if (fix?.kind === 'setIvDuration') {
    return {
      op: 'update',
