...
// 六則裡有五則的變數是**有條件的**：
//
//   - 「問壓好的時間」的 `{slots}` 要先排序、攤平成每一段一行，時間寫成
//     完整的一段（客戶要知道的是那天待多久，不是幾點報到）；匯入的舊資料
//     沒有時間，那一行就整段不印時間
...
export const TEMPLATES = [
  ...
  },
  {
    ...
    // `{slots}` 是**好幾行**，所以它自己要獨佔一行 —— 接在句子中間的話，
    // 第二段之後會黏在「請問可以嗎」前面（2026-09-04 改成逐段一行）。
    hint: '{slots} 是每一段各一行：日期、幾點到幾點、課程。它自己佔一行，不要接在句子中間',
    text: '{name}您好，{month} 月為您安排了：\n'
      + '{slots}\n'
      + '請問可以嗎？',
  },
  {
