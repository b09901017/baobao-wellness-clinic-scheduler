 ...
 * （客戶詳情、待辦中心、進度追蹤）列的本來就是整筆來訪。
 * 所以「畫哪幾段」是一條規則，規則寫在 domain。
 *
 * ## 兩條刻意的退路
 *
 * 1. **沒指定就是全部。** `undefined` 是「沒有人告訴我是哪一段」，
 *    不是「第 0 段」—— 那三頁第一張畫的就是那一天全部。
 *
 *    2026-09-10 起那三頁**點得下去**了（issue 10）：`focused` 回假時
 *    `visitReadHtml()` 把每一段畫成一列按鈕，點某一段就帶著它再問一次
 *    這一支。她指名要留先看到「那一天有哪幾段」這一層，所以退路本身沒有變
 *    —— 變的是呼叫端第二次會帶著段落序號回來。
