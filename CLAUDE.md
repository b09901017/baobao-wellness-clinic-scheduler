...
| 一筆來訪改得動的地方 | **只有日曆**（ADR-0056），日曆上的長按選單也算在那一個入口裡。**取消多一個入口**：壓表右上角的「批次取消」（ADR-0082）—— 它只取消、做不到別的事，「哪幾段取消得掉」走 `cancellableSlots()`。待辦中心、客戶詳情、進度追蹤那三張讀取卡片都沒有鉛筆，來訪紀錄那一列也不是連到編輯器的連結 —— 留一條繞過去的路，等於那個決定只做了一半 |
