...
| 一段來訪在畫面上叫什麼 | 只寫在 `domain/naming.js` 的 `slotName()`。**三種情境，三種讀者**：月檢視（她）只印器材別稱（`SIS`）、一般（她）是 `復能(SIS)`、**LINE 草稿（客戶）只有課程**（`復能`、`靜脈雷射`），一個器材字都不寫（ADR-0077）—— 所以器材的 `lineName` 畫不出來，名稱那一頁的器材列不給那一格。`slot.courseName` 是**快照不是顯示名稱** —— 名字是畫出來的，她改一次別稱全部畫面跟著改。設定 → 名稱怎麼寫 |
