...
| 一段來訪在畫面上叫什麼 | 只寫在 `domain/naming.js` 的 `slotName()`。**三種情境，三種讀者**：月檢視（她）只印器材別稱**再接上幾分鐘**（`SIS(60)`，只有課程有兩種以上規格時才接）、一般（她）是 `復能(SIS)`、**LINE 草稿（客戶）只有課程**（`復能`、`靜脈雷射`），一個器材字都不寫（ADR-0077）—— 所以器材的 `lineName` 畫不出來，名稱那一頁的器材列不給那一格。`slot.courseName` 是**快照不是顯示名稱** —— 名字是畫出來的，她改一次別稱全部畫面跟著改。設定 → 名稱怎麼寫 |
| 一筆額度叫什麼 | `domain/entitlements.js` 的 `poolName()` + `timedLabel()`，格式是 `復能 - 三選一（60）`／`復能 - SIS（60）`（**單買一台也帶著課程，後半用別稱**，ADR-0077）。它跟 `slotName()` 是**兩件事**：額度是「當初買了什麼」，時段是「那天做了什麼」。改格式要同時補 `legacyPoolNames()`，不然資料健檢的「復能額度還叫舊名字」認不出既有那幾筆，而她自己打的名字**不可以**被一顆按鈕改掉 |
...
| 備忘錄編輯那張卡的版面 | 卡片裡**除了內文那一格，每一項都 `flex: none`**（`.pbcard--edit > *`），內文是唯一的 `flex: 1 1 0`。少一條那一項就會被 flex 壓扁 —— `.emojirow` 被壓成 2px、textarea 被壓回 `min-height` 都發生過，而症狀是「東西在那裡但看不到也捲不到」。高度由 `.pbdeck[data-editing]` 給死，**不要用 JS 撐 textarea 的高度** |
