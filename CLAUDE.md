...
| 取消類的待辦（`取消 X`） | **逐段不逐天**：每一張帶 `slotIndexes`（它收的是哪幾段），一段還沒被任何一張蓋到才長新的一張 —— 規則只在 `domain/taskRules.js` 的 `cancelTasksFor()`，**整天取消、只取消幾段、確認框那一句三條路都走它**（ADR-0070）。以前「同一個系統含已勾掉的只准一張」，勾掉之後隔天再取消另一段就一張都不長。段落的位置穩得住是因為既有的段不會被移除（× 是取消）、新的一律接在尾巴（`hasNewSlots()` 也靠這個）—— **哪天加了「刪掉某一段」或「照時間重排」，這一格就會指到別段**。沒有 `slotIndexes` 的舊任務當成蓋住整天。只取消一段時 Examine／耀聖要不要收**她還沒定案**（2026-09-13，暫定要）|
...
| 日曆的標題、跳日期那一張 | 字只寫在 `domain/calendar.js` 的 `titleOf()`（月 `9月`、週 `9月 W2`、日 `9/18(五)`，**不是今年才帶年份** —— 「今年」由呼叫端傳），第幾週只寫在同一支的 `weekOfMonth()`（**週四落在哪個月就算哪個月**，2026-09-13 她同意的）。點標題那一張面板（`calendar.js` 的 `openDatePicker()`）的週列頭也讀它 —— 標題寫 W2、面板寫 W3 的話她不會知道哪個算數。iPad 橫式週檢視的一列是直排（`.weekgrid .timerow`），**不要改成橫向捲**：週檢視本身左右滑是換週（ADR-0020） |
