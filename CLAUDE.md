...
| 日曆上任何一種東西的顏色 | 要分辨的是六種：待確認／已確認／已完成／未到／個人行程／休假。加第七種之前先數一次還有沒有分得開的色相 —— 休假之所以走斜線紋而不是第六個顏色就是因為數不夠了（ADR-0039）。個人行程可以自己挑顏色，色票名單與客戶備註共用 `MARK_COLORS`，但值另有一組 `--evcolor-*`（小圓點的顏色當 11px 的字對比度不夠，見 ADR-0040） |
