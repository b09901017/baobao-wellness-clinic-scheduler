...
| 一筆來訪的讀取卡片要畫哪幾段 | `domain/visits.js` 的 `slotsToShow()`（ADR-0080）。**沒指定就是全部** —— 另外三頁列的本來就是整筆來訪；指到一個不存在的段落也退回全部（畫成空的比畫太多糟）。日／週那一列的 `data-open` 是 `visit:<id>:<第幾段>`，四個接線的地方走同一支 `parseOpen()`，有測試盯著沒有人自己 `split`。**「這一天還有另外 N 段」與長按選單的「共 N 段」都不要**（2026-09-09 她說不需要）；只有「取消一整天（N 段）」那個數字留著，那是煞車不是資訊 |
...
| 一筆來訪改得動的地方 | **只有日曆**（ADR-0056），日曆上的長按選單也算在那一個入口裡。**取消多一個入口**：壓表右上角的「批次取消」（ADR-0082）—— 它只取消、做不到別的事，「哪幾段取消得掉」走 `cancellableSlots()`。待辦中心、客戶詳情、進度追蹤那三張讀取卡片都沒有鉛筆，來訪紀錄那一列也不是連到編輯器的連結 —— 留一條繞過去的路，等於那個決定只做了一半。**而且只改她點的那一段**（ADR-0085）：`mountEdit()` 收 `slotIndex`，帶了就只畫那一段、沒有「＋新增一個時段」、沒有整筆的狀態卡與危險區。沒畫出來的那一段在 `readDraft()` 裡**整個回原本那個物件**，不要重組一份 —— `attended` / `status` 那幾格沒有欄位，重組會被寫死的 `null` 清掉 |
