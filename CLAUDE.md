...
| 一段來訪的狀態、或它算不算數 | **時段才是原子單位**（ADR-0081）。五種狀態都在 `slot.status` 上，讀一律走 `domain/visits.js` 的 `slotStatus()`（沒有那一格就退回整筆，舊資料一個字都不用改）。整筆那個 `status` 是**推導出來又存起來的**（`visitStatusFrom()`）—— 索引、Rules、試算表、備份四個地方讀它，所以改了時段一定要重推，資料健檢有一列盯著。次數走 `slotOutcome()`，取消的那一段回 `null`（＝不佔次數）。「哪幾段還算數」只有一支 `isLiveSlot()`，掛號／紀錄／療程單／後果說明全部走它。**`bookingSystemsForVisit()` 刻意不濾** —— 整筆取消時每一段也是 cancelled，濾掉就一張「取消 Abovee」都長不出來，有測試釘著。**畫面上的抬頭一律印那一段的**（`statusForCard()`，ADR-0085）—— 整筆那一個是推導的，加一段沒問過客人的進去就會退回「待確認」，而她點的可能是早上那段已經談定的 |
...
| 一筆來訪的讀取卡片要畫哪幾段 | `domain/visits.js` 的 `slotsToShow()`（ADR-0080）。**沒指定就是全部** —— 另外三頁列的本來就是整筆來訪；指到一個不存在的段落也退回全部（畫成空的比畫太多糟）。**但那三頁的每一段自己是一列可以點的**（2026-09-10）：`focused` 為假而且不只一段時畫成 `<button>` 帶 `data-open`，點下去用同一支再開一張只有那一段的。日／週那一列的 `data-open` 是 `visit:<id>:<第幾段>`，**五個接線的地方走同一支 `parseOpen()`**（日曆四處 ＋ 那三頁共用的 `wireReadSlots()`），有測試盯著沒有人自己 `split`。三頁的卡片副標一律 `statusForCard(visit, focus)`。**「這一天還有另外 N 段」與長按選單的「共 N 段」都不要**（2026-09-09 她說不需要）；只有「取消一整天（N 段）」那個數字留著，那是煞車不是資訊 |
...
| 一筆來訪改得動的地方 | **只有日曆**（ADR-0056），日曆上的長按選單也算在那一個入口裡。**取消多一個入口**：壓表右上角的「批次取消」（ADR-0082）—— 它只取消、做不到別的事，「哪幾段取消得掉」走 `cancellableSlots()`。待辦中心、客戶詳情、進度追蹤那三張讀取卡片都沒有鉛筆，來訪紀錄那一列也不是連到編輯器的連結 —— 留一條繞過去的路，等於那個決定只做了一半。**而且只改她點的那一段**（ADR-0085）：`mountEdit()` 收 `slotIndex`，帶了就只畫那一段、沒有「＋新增一個時段」、**沒有整筆的狀態卡與危險區**（2026-09-10 真的拿掉了，摺起來不算）。它們的第二條路是**日曆讀取卡片底下那一顆「改這一天」**（`data-edit-day`，開的是沒有 `slotIndex` 的編輯器）—— 拿掉那一摺而不補這一顆就違反 ADR-0060。沒畫出來就不要接線：`wireStatus()` / `wireDangerZone()` 走 `wholeVisit`，接在 null 上會讓整頁停在「載入中…」而且一句錯誤訊息都沒有。沒畫出來的那一段在 `readDraft()` 裡**整個回原本那個物件**，不要重組一份 —— `attended` / `status` 那幾格沒有欄位，重組會被寫死的 `null` 清掉 |
...
| 畫面上一段常駐的說明 | **先想能不能收進 `ui/components/tip.js` 的 `tip()`**（點了才浮出來的小泡泡，ADR 見 issue 08）。她 2026-09-10：「非常占版面，一眼看下去非常雜亂，會視覺疲勞」。**但會改變寫入結果的警告不可以收進去** —— 撞期、醫療禁忌、次數不夠那幾種她沒點開就等於沒看到，而她已經按下去了。紅線寫在 `tests/tip-red-lines.test.js` 裡，掃原始碼盯著。沒話講就一個像素都不佔（空字串回空字串）|
...
| 一筆來訪的讀取卡片 | `ui/views/calendar.js` 的 `visitReadHtml()`，**四個畫面共用**（日曆、客戶詳情、待辦中心、進度追蹤，ADR-0018、0056）。哪幾段畫得出來走 `slotsToShow()`（ADR-0080）。「這一場的待辦」那一塊在裡面，所以四頁一起長；備忘錄那一塊與「改這一天」那一顆接在**外面**，只有日曆有（ADR-0088）。**沒指定哪一段時每一段自己是一列可以點的**，接線走 `wireReadSlots()`（也在 `calendar.js` 裡，跟 `parseOpen()` 同一支）—— 三頁各寫一份 `split(':')` 遲早有一份忘了取第三格。那一塊**只給看不給勾** —— 一個 `<input type="checkbox">` 都不可以有 |
