...
| 日曆的標題、跳日期那一張 | 字只寫在 `domain/calendar.js` 的 `titleOf()`（月 `9月`、週 `9月 W2`、日 `9/18(五)`，**不是今年才帶年份** —— 「今年」由呼叫端傳），第幾週只寫在同一支的 `weekOfMonth()`（**週四落在哪個月就算哪個月**，2026-09-13 她同意的）。點標題那一張面板（`calendar.js` 的 `openDatePicker()`）的週列頭也讀它 —— 標題寫 W2、面板寫 W3 的話她不會知道哪個算數。週檢視每一天的抬頭寫 `9/7` |
