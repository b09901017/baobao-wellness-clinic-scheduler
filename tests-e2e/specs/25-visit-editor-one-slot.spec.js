...
test('V2 存那一段，沒畫出來的那幾段一個欄位都不動', async ({ app, page }) => {
  ...
  // 改了時間＝取消＋重新排（ADR-0108）：先講要回去做什麼
  await app.ok();
  await app.saved();

  const saved = await app.readDoc('visits', 'v-three');

  expect(saved.slots, '三段都還在，新的時間接在尾巴').toHaveLength(4);
  expect(saved.slots[1].status, '她改的那一段取消了').toBe('cancelled');
  expect(saved.slots[3].startsAt, '新的那一段').toBe('14:00');
  expect(saved.slots[3].endsAt, '結束時間跟著時長重算').toBe('14:30');
  expect(saved.slots[3].status, '要再問客人一次').toBe('pending_confirm');
...
// ---------- 改期＝取消＋重新排（prelaunch-audit-2026-09-23/issues/13，ADR-0108） ----------
//
// 她 2026-09-23：「我希望會提醒回 Abovee／Examine／耀聖改時間以及回去取消已經掛好的號等等」

function seedSettledRehab() {
  const registered = (id, kind) => ({
    path: 'tasks', id,
    data: {
      customerId: 'cust-r', customerName: '王小明', kind, dueDate: addDays(DAY, -1), visitId: 'v-r',
      done: true, doneAt: addDays(DAY, -1), autoGenerated: true, note: null, slotIndexes: [0],
    },
  });
  return [
    ...masterDocs(),
    customer({ id: 'cust-r', name: '王小明' }),
    entitlement('cust-r', {
      id: 'ent-rehab', label: '復健科醫師門診', type: 'single', courseId: 'course-rehab', totalQty: 6, bookedCount: 1, durationMin: 30,
    }),
    visit({
      id: 'v-r', customerId: 'cust-r', customerName: '王小明', date: DAY, status: 'confirmed', confirmedAt: DAY,
      slots: [{
        ...slot({ courseId: 'course-rehab', entitlementId: 'ent-rehab', startsAt: '14:00', endsAt: '14:30', doctorId: 'staff-dr-xia' }),
        status: 'confirmed',
      }],
    }),
    registered('t-ex', 'Examine'),
    registered('t-yao', '耀聖'),
  ];
}

test('V12 已談定、掛好號的那一段改時間 → 先講要回去做什麼，存完舊的取消、新的待確認', async ({ app, page }) => {
  await app.seed(seedSettledRehab());
  await app.signIn('/calendar');
  await openDay(app, page);
  await openEditorForSlot(app, page, 'v-r', 0);

  await page.locator('input[name="s0-start"]').fill('16:00');
  await page.locator('button[type="submit"]').first().click();

  const said = await app.dialogText();
  for (const k of ['取消 Abovee', '取消 Examine', '取消 耀聖', '再問客人']) expect(said, k).toContain(k);
  await app.ok();
  await app.saved();

  const saved = await app.readDoc('visits', 'v-r');
  expect(saved.slots.map((s) => [s.startsAt, s.status])).toEqual([['14:00', 'cancelled'], ['16:00', 'pending_confirm']]);
  expect(saved.status, '新的那一段還沒問客人').toBe('pending_confirm');
  const open = (await app.readAll('tasks')).filter((t) => !t.deletedAt && !t.done).map((t) => t.kind).sort();
  expect(open).toEqual(['取消 Abovee', '取消 Examine', '取消 耀聖'].sort());
});

test('V13 只記一句話 → 一道確認都不多跳、一張待辦都不多長', async ({ app, page }) => {
  await app.seed(seedSettledRehab());
  await app.signIn('/calendar');
  await openDay(app, page);
  await openEditorForSlot(app, page, 'v-r', 0);
  ...
  const saved = await app.readDoc('visits', 'v-r');
  expect(saved.slots.map((s) => [s.startsAt, s.status, s.note])).toEqual([['14:00', 'confirmed', '客人說會晚到']]);
  expect((await app.readAll('tasks')).filter((t) => !t.deletedAt && !t.done)).toEqual([]);
});
