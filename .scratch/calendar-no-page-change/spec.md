...
來訪身上的 `note`（SPEC 第 5.3 節）**只有壓表那一頁填得到**，來訪編輯器裡沒有那個欄位
—— 從日曆或客戶詳情進去改一筆，那句話看得到、改不了。補上之後才發現一件事：
這一頁的 `change` 事件會重畫整張表單（其餘欄位都是用點的，點完本來就要重算），
而這是表單上第一個自由輸入的欄位 —— 她打完字、手指按向「儲存」的那一刻表單被換掉，
按下與放開落在兩個不同的元素上，**那一下不算數**。改成只有這一欄不觸發重畫。

長度上限移到 `domain/visits.js` 的 `NOTE_MAX`，壓表那一頁改用同一個，
`tests/imports.test.js` 盯著兩邊都不許再寫死數字。

畫面上不叫它「備註」—— 那個詞在 `CONTEXT.md` 是客戶身上的彩色標記。
兩處都改成「這一次記一句」。
...
`npm test`（505 支）之外，這一輪同樣用假的 Firebase SDK 把真的畫面跑起來截圖看
（Playwright + 攔截 `firebasejs` 與 `firebase-config.js`），十八個步驟走完六件事
加上那個缺口。抓到四個只有在瀏覽器裡才看得到的問題：

- 抽屜換內容時 `note` 沒跟著換，編輯器上面寫著「1 件事。點一筆看細節」
- 那一天的件數漏算有時間的個人行程（只加了整天的）
- 編輯器掛進抽屜之後，抬頭與編輯器自己的第一列都寫著客戶姓名
- 打完那一句話直接按「儲存」，那一下不算數（表單在 `change` 時整個重畫）
