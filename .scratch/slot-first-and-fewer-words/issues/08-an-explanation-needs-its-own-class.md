...
## 做出來的跟這裡寫的不一樣（2026-09-12）

**沒有長出 `.explain` 這個 class。** 原本的計畫是「先只做分類、不改行為」，
再逐批轉；實際上分類與轉換是同一輪做完的，所以一個只標「留下來的那幾段」
的 class 標不到什麼東西。

換成的是 `tests/fewer-words.test.js` 的**棘輪**：每一支檔案還剩幾段常駐說明
寫在 `KEEP` 裡**並且附理由**，只准往下。分類的紀錄就在那幾行理由裡。

**棘輪答得出「有沒有變多」，答不出「收進泡泡的那一句是不是說明」** ——
後者由同一支測試的另外兩條補：收起來的那八段字要出現在 `tip()` 的呼叫裡
（issue 09 的判準），而 `muted dim` 那十四處一段都沒收的理由也列在裡面。
